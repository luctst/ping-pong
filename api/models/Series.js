...
Series.watch().on('change', async function (newData) {
    if (newData.operationType === 'update') {
        ...
        const playersSession = await playersModel
            .where('_id').in([
                mongoose.Types.ObjectId(newData.updateDescription.updatedFields[fieldUpdate].players.winner),
                mongoose.Types.ObjectId(newData.updateDescription.updatedFields[fieldUpdate].players.looser)
            ]);
        
        await Promise.all(playersSession.map(async p => {
            ...
            if (newData.updateDescription.updatedFields[fieldUpdate].players.winner.toString() === p._id.toString()) {
                fieldsToUpdate.win = p.win + 1;
            };

            await playersModel.updateOne({_id: mongoose.Types.ObjectId(p._id)}, { ...fieldsToUpdate });
        }))
    }
});
